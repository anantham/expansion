...
| **integrity-scan** | `/expansion:integrity-scan` | Audits whether the code tells the truth about itself — names that lie (`get_*` with side effects, `validate_*` that never rejects), docstrings/comments claiming guarantees the code lacks, comments citing mechanisms that exist but are **neutralized**, one concept under many names, the same logic forked N times, band-aid layering, and stale scaffolding. |
